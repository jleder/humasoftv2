...
//Escriba tus variables
    ...
    function sendmailPropActualizada() {
        ...
        $mail->addAddress('isandoval@humagroperu.com', "Isabel Sandoval");
    ...
    function sendmailPropAprobtoPrueba() {
        ...
        $mail->Body = $body;
        $mail->IsHTML(true);
        $mail->addAddress('sistemas@humagroperu.com', "Juan Leder");
        $mail->Send();
    }
    ...
    function sendmailMsjAprobacionACaDESP() {
        ...
        //$mail->addCC('reportes@humagroperu.com', "Salvador Giha");
        //$mail->addBCC('sistemas@humagroperu.com', "Juan Leder"); //Copia oculta
//        $mail->addBCC('acomercial@humagroperu.com', "Jennifer Rivera");
        if($mail->Send()){
            return true;
        }else{
            return false;
...
//        $mail->addBCC('acomercial@humagroperu.com', "Jennifer Rivera");
    ...
    function getItemByCodProp() {
        $sql = "SELECT coditem, codprop, itemdesc, coduse, fecreg, descuento, pcc, pca, precioambt, fa, ha, plantilla, nitrogeno, pud, estado, modificado, incluyeigv, valigv, pup, verfc, pyk from propdb12 where codprop = '$this->codprop' order by fecreg asc;";
        $result = $this->consultas($sql);
        $lista = $this->n_Arreglo($result);
        return $lista;
    }
    ...
    function insertPropxAprob() {
        $sql = "INSERT INTO propdb10(codprop, codcliente, nomcliente, contactos, asunto, descripcion, cultivo, variedad, efenologica, coduse, fecreg, vendedor, elaboradopor, demo, obs_atec, fpago, trato, numdespacho, condiciones, codvendedor, zona) VALUES('$this->codprop', '$this->codcliente', '$this->nomcliente', '$this->contactos', '$this->asunto',  '$this->descripcion', '$this->cultivo', '$this->variedad', '$this->efenologica', '$this->domuser', now(), '$this->nomvendedor', '$this->elaboradopor', '$this->demo', '$this->obs_atec', '$this->fpago', '$this->trato', '$this->numdespacho', '$this->condiciones', '$this->codvendedor', '$this->zona');";
        $result = $this->consultas($sql);
        return $result;
    }
    ...
    function insert_propdb14($codprod, $cantidad1, $cantidad2, $umedida1, $umedida2, $preciou, $preciodcto, $preciototal, $factorb, $ordendesp, $congelado) {
        $sql = "INSERT INTO propdb14(codprop, codprod, coditem, cantidad1, cantidad2, umedida1, umedida2, preciou, preciodcto, preciototal, factorb, ordenprod, coduse, congelado) values ('$this->codprop', '$codprod', '$this->coditem', '$cantidad1', '$cantidad2', '$umedida1', '$umedida2', '$preciou', '$preciodcto', '$preciototal', '$factorb', '$ordendesp', '$this->domuser', '$congelado');";
        $result = $this->consultas($sql);
        return $result;
    }
    ...
    //insert propdb12
    function insertItem() {
        $sql = "INSERT INTO propdb12(coditem, codprop, itemdesc, coduse, fecreg, descuento, pcc, pca, precioambt, fa, ha, nitrogeno, plantilla, pud, estado, modificado, incluyeigv, valigv, pup, verfc, pyk) VALUES('$this->coditem', '$this->codprop', '$this->itemdesc', '$this->domuser', now(), '$this->descuento', '$this->pcc', '$this->pca', '$this->precioAMBT', '$this->fa', '$this->ha', '$this->nitrogeno', '$this->plantilla', '$this->pud', '$this->estadoitem', '$this->modificado', '$this->incluyeigv', '$this->valigv', '$this->pup', '$this->verfc', '$this->pyk');";
        $result = $this->consultas($sql);
        return $result;
    }
    ...
    function sendmailPropxAprob() {
        require '../site/PHPMailer/class.phpmailer.php';
        $url1 = '<a target="_blank" href="http://humagro.ddns.net:8070/humasoft/site/_propuestav2_aprob02_verweb.php?cod=' . $this->codprop . '">' . $this->codprop . '</a>';
        $url2 = '<a target="_blank" href="http://humagro.ddns.net:8070/humasoft/site/_propuestav2.web.php">Ver Todas las Propuestas</a>';
...
// convert in PDF
    ...
    function bodymail_propinsert_gerente() {
        $url1 = 'http://humagro.ddns.net:8070/humasoft/site/_propuestav2_aprob02_verweb.php?cod=' . $this->codprop;
        $url2 = 'http://humagro.ddns.net:8070/humasoft/site/_propuestav2.web.php';
    ...
    function sendmailMsjAprobacion() {
        ...
        $mail->Body = $body;
        $mail->IsHTML(true);
        $mail->addAddress('areatecnica@humagroperu.com', "Kateryn Mendoza Hinostroza");
        //if ($estado == 'APROBADO') {
    ...
    function sendmailMsjAprobacionATaAC() {
        ...
        $mail->IsHTML(true);
        $mail->addAddress($this->mailpara);
        //$mail->addAddress('sistemas@humagroperu.com', "Jenifer Rivera");        
    ...
    function getCondiciones() {
        $condiciones = array(
            ...
            "Si tiene algún comentario no dude en comunicarse con nosotros al (01)368-3531, RPC 962383835, escríbenos a acomercial@humagroperu.com o visite nuestra página web www.NutricionMicroCarbono.com.",
    ...
    function obtenerZonaxVendedor(){
        $sql = "SELECT p.codzona, z.zona FROM hs_epddb02 as p 
                JOIN ubzona as z ON z.codzona = p.codzona
                WHERE p.codpersona = '$this->codvendedor';";
        $result = $this->consultas($sql);
        $lista = $this->n_Arreglo($result);
        return $lista;
