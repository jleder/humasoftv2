                            ...
                            foreach ($listaprop as $valor) {
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                        //Estado de Despachos
                                        $despa->__set('codprop', $valor['codprop']);
                                        $listdesp = $despa->despachosByCodprop();
                                        $nlistdesp = count($listdesp);
                                        if ($nlistdesp > 0) {
                                            $i = 0;
                                            foreach ($listdesp as $desp) {
                                                $i++;
                                                $det->__set('coddesp', $desp['coddesp']);
                                                $listdet = $det->getLastDetDespByCoddesp();
                                                if ($listdet) {
                                                    $estado = $despa->getcolorestado($listdet[3]);
                                                    echo '<p>' . $desp['descripcion'] . ': ' . $estado . ' ' . date('d/m/Y', strtotime($listdet[2])) . '</p>';
                                                } else {
                                                    echo '<p>' . $desp['descripcion'] . ': <span style="color: orange;">Desconocido</span></p>';
                                                }
                                            }
                                        } else {
                                    ...
                                    <td style="text-align: center;">
                                        ...
                                        <a class="btn2 btn-icon" href="#" onclick="cargar('#divpropuesta', '_propuestav2_areacomercial.php?codprop=<?php echo trim($valor['codprop']); ?>')" title="Área Comercial"><img src="img/iconos/icon-c.png" style="height: 15px; width: 15px;"/></a>
                                        <a class="btn2 btn-icon" href="#" onclick="javascript:cargar('#divpropuesta', '_propuestav2_detdespacho.php?cod=<?php echo trim($valor['codprop']); ?>')" title="Despachos"><img src="img/iconos/icon-d.png" style="height: 15px; width: 15px;"/></a>
                                        <a class="btn2 btn-icon" href="_propuestav2_archivo.php?codprop=<?php echo trim($valor['codprop']); ?>" title="Archivos"><img src="img/iconos/file-icon.png" style="height: 15px; width: 15px;"/></a>
